...
#   - /session-memory-update command (manual, user-triggered)
#
# INPUT:
...
for cmd in jq python3; do
  if ! command -v "$cmd" &>/dev/null; then
    ...
    echo "Error: session-memory plugin requires '$cmd' to be installed" >&2
    exit 1
  fi
done
...
if [ -z "$CLAUDE_BIN" ]; then
  ...
  if command -v claude &>/dev/null; then
    ...
    echo "Error: session-memory plugin requires 'claude' CLI to be installed" >&2
    exit 1
  fi
fi
